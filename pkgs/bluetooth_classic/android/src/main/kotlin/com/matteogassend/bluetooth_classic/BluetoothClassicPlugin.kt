  ...
  private fun connect(result: Result, deviceId: String, serviceUuid: String) {
    ...
    if (btAdapter.isEnabled) {
        try {
            ...
            android.util.Log.i("Bluetooth Connection", "Device found: $deviceId")

            val SERIAL_UUID = UUID.fromString(serviceUuid)

            try {
                socket = device.createRfcommSocketToServiceRecord(SERIAL_UUID)
                android.util.Log.i("Bluetooth Connection", "RFCOMM socket created")
            } catch (e: Exception) {
                android.util.Log.e("Bluetooth Connection", "Error creating socket", e)
            }
            ...
            try {
                ...
                android.util.Log.e("Bluetooth Connection", "Connection failed: ${e.message}")
                try {
                  ...
                  try {
                    socket!!.connect()
                    android.util.Log.i("Bluetooth Connection", "Fallback socket connected")
                  }catch (e: IOException){
                    socket = device.javaClass.getMethod("createRfcommSocket", Int::class.javaPrimitiveType)
                    ...
                    android.util.Log.e("Bluetooth Connection", "Couldn't establish Bluetooth connection", e2)
                    publishBluetoothStatus(0)
        ...
        android.util.Log.e("Bluetooth Connection", "Bluetooth is not enabled")
